...
if __name__ == "__main__":
	...
	argparser.add_argument("--price",dest="price",metavar = "YYYY-MM-DD-HH:MM",default = None,help='These are the tickers for which data will be gathered and accumulated')
	argparser.add_argument("--signal",dest="signals",metavar = "YYYY-MM-DD-HH:MM", default = None,help='This call will return the most recent data for all stocks that were subscribed at the given time')
	...
	argparser.add_argument("--del_ticker",dest="delticker",metavar='AAPL',help='This call will remove monitoring for a given ticker.Return codes 0=success, 1=server error, 2=ticker not found')
	argparser.add_argument("--add_ticker",dest="newticker",metavar='AAPL',help='This will inform the server to start monitoring this ticker and report signals for this ticker. Return code 0=success, 1=error, 2=invalid ticker')
	argparser.add_argument("--reset",dest="reset",action='store_const',const = True, default = None,help='Informs the server to reload all data from source vendors and recalculates signals')
